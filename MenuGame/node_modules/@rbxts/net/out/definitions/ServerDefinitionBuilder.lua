...
do
	...
	function ServerDefinitionBuilder:_CreateOrGetInstance(id, declaration)
		...
		if declaration.Type == "Function" then
			local func
			if remoteFunctionCache[id] ~= nil then
				return remoteFunctionCache[id]
			else
				if declaration.ServerMiddleware then
					func = ServerFunction.new(id, declaration.ServerMiddleware)
				else
					func = ServerFunction.new(id)
				end
				CollectionService:AddTag(func:GetInstance(), "NetDefinitionManaged")
				local _func = func
				remoteFunctionCache[id] = _func
				local _result = self.globalMiddleware
				if _result ~= nil then
		...
		elseif declaration.Type == "AsyncFunction" then
			...
			if remoteAsyncFunctionCache[id] ~= nil then
				return remoteAsyncFunctionCache[id]
			else
				if declaration.ServerMiddleware then
					asyncFunction = ServerAsyncFunction.new(id, declaration.ServerMiddleware)
				else
					asyncFunction = ServerAsyncFunction.new(id)
				end
				CollectionService:AddTag(asyncFunction:GetInstance(), "NetDefinitionManaged")
				local _asyncFunction = asyncFunction
				remoteAsyncFunctionCache[id] = _asyncFunction
			end
			local _result = self.globalMiddleware
			if _result ~= nil then
				local _arg0_1 = function(mw)
		...
		elseif declaration.Type == "Event" then
			local event
			-- This should make certain use cases cheaper
			if remoteEventCache[id] ~= nil then
				return remoteEventCache[id]
			else
				if declaration.ServerMiddleware then
					event = ServerEvent.new(id, declaration.ServerMiddleware)
				else
					event = ServerEvent.new(id)
				end
				CollectionService:AddTag(event:GetInstance(), "NetDefinitionManaged")
				local _event = event
				remoteEventCache[id] = _event
			end
			local _result = self.globalMiddleware
			if _result ~= nil then
				local _arg0_1 = function(mw)
		...
		elseif declaration.Type == "Messaging" then
			local event
			if messagingEventCache[id] ~= nil then
				return messagingEventCache[id]
			else
				event = ExperienceBroadcastEvent.new(id)
				local _event = event
				messagingEventCache[id] = _event
			end
			return event
		elseif declaration.Type == "ExperienceEvent" then
			local event
			if messagingServerEventCache[id] ~= nil then
				return messagingServerEventCache[id]
			else
				event = ServerMessagingEvent.new(id)
				local _event = event
				messagingServerEventCache[id] = _event
			end
			return event
		else
